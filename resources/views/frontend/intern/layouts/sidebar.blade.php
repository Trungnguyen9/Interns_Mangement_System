<aside class="ims-sidebar">
    ...
    <a href="{{ url('/internpage/dashboard') }}"
        class="sidebar-item {{ request()->routeIs('frontend.intern') ? 'active' : '' }}">
        <i class="fa-solid fa-gauge icon"></i> Dashboard
    </a>
    <a href="{{ url('/internpage/profile') }}"
        class="sidebar-item {{ request()->routeIs('frontend.profile.index') ? 'active' : '' }}">
        <i class="fa-solid fa-user-circle icon"></i> Hồ sơ
    </a>
    <a href="{{ url('/internpage/tasks') }}"
        ...
        @if ((auth()->user()->internProfile->tasks->count() ?? 0) > 0)
            <span
                class="badge-count">{{ auth()->user()->internProfile->tasks->whereNotIn('status', ['Done', 'Review'])->count() }}</span>
        @endif
    </a>
    <a href="{{ url('/internpage/reports') }}"
